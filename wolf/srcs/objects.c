...
/*   Updated: 2019/12/06 23:40:29 by saneveu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		get_objangle(t_wolf *data, t_object *list)
{
	...
	list->data.spriteHeight = ft_abs((int)(W_HEIGHT / (list->data.transformy)));
	...
	list->data.spriteWidth = ft_abs((int)(W_HEIGHT / list->data.transformy));
...
static void		display_object(t_wolf *d, t_object *l)
{
	...
	while (sx < l->data.width)
	{
		texX = (int)(256 * (sx - (-l->data.spriteWidth / 2 + l->data.spriteScreenX))
			* l->sprite.img->w / l->data.spriteWidth) / 256; 
		if (l->data.transformx > 0 && sx > 0 && sx < W_WIDTH && l->data.transformy < d->map.depth_buffer[sx])
		{
			...
			while (sy < l->data.drawEndY)
			{
        		...
        		put_pixel(d->screen, sx, sy, get_pixel_ray(d, l->si, texX, texY));
				sy++;
			}
		}
		sx++;
